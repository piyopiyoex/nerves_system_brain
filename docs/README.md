...
examples/hello_kiosk
    Elixir アプリ / KIOSK UI / 入力 / SSH / NIF
             │
             ▼
nerves_system_brain
    Buildroot rootfs / Erlang/OTP / erlinit / OS 設定
             │
             ▼
brain-hackers の起動基盤
...
`nerves_system_brain` が提供する。アプリケーション例の `hello_kiosk` は同じリポジトリに
置くが、システム側から依存しない。
...
- 開発用の標準通信経路は USB-NCM とする。
- 現在は `nerves_system_br` の standalone build を使用する。
...
- Mix から利用する通常の Nerves System package / system artifact
- `fwup` による firmware 生成・更新
- SD 配備フローの統合
