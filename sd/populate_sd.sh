...
sd_require_root
sd_require_commands lsblk readlink mountpoint findmnt mount umount mkfs.ext4 tar cp install sync du mktemp rmdir mkdir cut
...
DTB_SOURCE="$SCRIPT_DIR/imx28-pwsh6-peripheral.dtb"
[ -f "$DTB_SOURCE" ] || sd_die "USB peripheral 用 DTB が見つかりません: $DTB_SOURCE"

sd_confirm_device "$DEV" "$PART_BOOT の DTB を置き換え、$PART_ROOT を ext4 で初期化します"
...
install -m 644 "$DTB_SOURCE" "$MNT_BOOT/imx28-pwsh6.dtb"
printf 'p1: USB peripheral 用 DTB を配置しました\n'
...
# nerves_system_br は Erlang/OTP を target ではなく staging にのみ配置する。
# 現在の bring-up 用 rootfs で erl を起動できるよう、明示的に追加する。
